...
impl ActiveTextInsertion {
    ...
    async fn wait_for_stream_drain(&self) {
        loop {
            ...
            let scheduled = self
                ...
                .expect("text insertion lock poisoned")
                .scheduled;
            log::debug!("[insert] wait_for_stream_drain: scheduled={scheduled}");
            if !scheduled {
                break;
            }
            notified.await;
    ...
    async fn finish(self: &Arc<Self>, final_text: String) -> Result<InsertOutcome, BackendError> {
        ...
        self.task_spawner.spawn(Box::pin(async move {
            log::debug!("[insert] finish_committed task: started");
            let result = insertion.finish_committed(final_text).await;
            log::debug!("[insert] finish_committed task: done");
...
impl OpenLessBackend {
    ...
    async fn finish_text_insertion(
        ...
        log::debug!("[insert] preparation resolved, calling insertion.finish ({} chars)", final_text.chars().count());
        let result = insertion.finish(final_text).await;
        log::debug!("[insert] insertion.finish returned: {:?}", result.as_ref().map(|o| match o { crate::ports::InsertOutcome::Inserted => "inserted", crate::ports::InsertOutcome::PasteSent => "paste_sent", crate::ports::InsertOutcome::CopiedFallback => "copied_fallback" }));
